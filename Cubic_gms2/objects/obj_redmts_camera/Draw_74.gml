...
repeat (i) {
	...
	if (surface_exists(par_ls_surfs[i])) {
		...
		var xl = -layer_speed*(x - parallax_origo_x) + bb*obj_display_manager.view_zoom - left_bor;
		...
		} else draw_surface_stretched(par_ls_surfs[i], xl/obj_display_manager.view_zoom, yl/obj_display_manager.view_zoom, onscreen_w, onscreen_h);
	}
}
//
...
if (surface_exists(surf_room)) {
	...
	var xl = -layer_speed*(x - parallax_origo_x) + bb*obj_display_manager.view_zoom  - left_bor;
	var yl = -layer_speed*(y - parallax_origo_y) - top_bor;
    draw_surface_stretched(surf_room, xl/obj_display_manager.view_zoom , yl/obj_display_manager.view_zoom , onscreen_w, onscreen_h);
}
